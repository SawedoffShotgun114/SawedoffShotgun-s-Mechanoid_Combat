<p>
  <img src="About/SS_Icon.png" height="128" align="left">
  <h1 align="right">SawedoffShot's Mechanoid_Combat</h1>
</p>

<p align="right">  
  <a href="https://github.com/AlohaOe114/SawedoffShotgun-s-(Placeholder)/releases">  
    <img src="https://img.shields.io/badge/-Github发布页(获取最新版本)-gray?style=for-the-badge&logo=github">  
  </a>  
  <a href="mailto:youngturksfree@outlook.com">  
    <img src="https://img.shields.io/badge/-电邮联系我们-blue?style=for-the-badge&logo=mail">  
  </a>  
</p>  

# 免责声明
- 本作品内容不过是赶工而作，基于作者个人经验和记忆，可能存在不全、不周甚至错误的情况。
- 本作者对 "尚未足够地观看本文以了解mod特性后而自行操作" 所造成的一切后果概不负责。

------------------------------------------

 - 自 20240826
 - 版本: 1.0.0.1 (支持 Rimworld 1.5/1.4)

## 描述

 一个 100% XML 的 Rimworld Mod，旨在添加一些可能适合现实生活和 Vanilla/CE 游戏环境的新战斗类机械族。    
 本Mod具体内容的"关键词"：    
  “近战“ 或 ”反蜈蚣”    

 "王工，恁那些铁玩意弄好了木有？"    

## 兼容性:
游戏中途加入-安全    

==**!!CombatExtended(CE) Compatible/CE兼容/战斗扩展兼容!!**==    

原版Core兼容     
人物编辑器(CharacterEditor)(CE)兼容     
CheatEngine(CE) 也许兼容    

## 特性: (对作品内容的数值/特性的介绍)

 ##### Vanilla(原版)，也但有 CE 补丁：
 - 刽子手 (近战; 反蜈蚣):    
 （**警告：它很有可能一击杀死玩家方的生物与机械，所以要与 刽子手 保持距离。    
 （**警告：在你下载这个模组之前，你最好知道如何对抗蜈蚣（火箭发射器和EMP）。或者你可以直接“cheerypicker（一个 mod 可以”删除“一些你不想要的东西）”掉”刽子手“**） 
  一种巨大但仍然相对灵活快速的内置护盾发生器的近战战斗机械，常用一种用于近战战斗(或攻城)的巨型充能剑做武器。\n\n其内置烟雾发生器，且具有短程跳跃能力（2-3 击可毁坏一堵花岗岩墙） 

 ##### CE独占：    
 (占位符)    

### [如果您认为本作品中的一些物品做得不好/无意义，那作者诚挚推荐CherryPicker这个mod，它可以选择性"删除"你不想要的部分 (CherryPicker原作着: Owlchemist (致敬传奇modder猫头鹰，牢鹰想你了，o7) (https://steamcommunity.com/sharedfiles/filedetails/?id=2633276599))](https://steamcommunity.com/sharedfiles/filedetails/?id=3230046902)

### 物品数据:

 基本数据：
  原版数据：

| 种族  | 生命值          | 护盾 生命值/恢复速度 | 移速  | 是否免疫心灵冲击/心灵狂暴 | 技能         | 锐器护甲          | 钝器护甲           | 带宽消耗 | 科技需求             | 制造成本                                |
|-----|--------------|-------------|-----|---------------|------------|---------------|----------------|------|------------------|-------------------------------------|
| 刽子手 | 4.5\*150=675 | 400%/50%每秒  | 4.0 | 是的，免疫         | 短程跳跃, 瞬爆烟雾 | 120%/18+27RHA | 40%/9+65.25MPa | 5    | 高级机械族制造 (Tier 3) | 350*钢铁, 350*玻璃钢, 10*零部件, 2*同步信号芯片, 1*高级次生核心 |

| 武器    | 质量   | 体积  | 锐器穿透(武器握柄/刺/劈砍) | 钝器穿透(武器握柄/刺/劈砍) | 冷却时间(武器握柄/刺/劈砍) | 每击伤害(武器握柄/刺/劈砍)               | 市场价值 | 平均每秒伤害 | 科技需求 |
| ----- | ---- | --- | --------------- | --------------- | --------------- | ----------------------------- | ---- | ------ | ---- |
| 机械族巨剑 | 6.12 | 32  | 30%/102%/75%    | -               | 2/1.65/1.45     | 16(+5破拆)/72(+16破拆)/160(+20破拆) | 8330 | 97.75  | -    |

  CE数据：

| 武器    | 质量   | 体积 | 锐器穿透(武器握柄/刺/劈砍) | 钝器穿透(武器握柄/刺/劈砍) | 冷却时间(武器握柄/刺/劈砍) | 每击伤害(武器握柄/刺/劈砍)               | 市场价值 | 平均每秒伤害 |
|-------| ---- | ---- | ------------------------------------- | ------------------------------------------------ | ------------------------------- | ----------------------------------------------- | ----------- | --------------------- |
| 机械族巨剑 | 6.12 | 32   | 102.67RHA/73.67RHA                    | 5.2MPa/7.33MPa/44.1MPa                           | 1.5/1.38/1.22                   | 16(+5破拆)/72(+16破拆)/160(+20破拆) | 8330        | 80.58                 |

------------------------------------------
 
## 请注意:
 作者玩CE玩的，可能不够熟悉原版环境

## 多语言:
 - English    
 - ChineseSimplified    
 - ChineseTraditional    

## FAQ: （常见问题解答）
 Q1: CE compatible?/兼容CE吗?    
 A1: CE compatible./兼容CE ，而且一些物品是CE版独有的(也许作者以后会继续更新相关的物品)   

 Q2: 包括但不限于以下的贴图方面的问题: "贴图风格不统一", "贴图丑", 等等...    
 A2: 作者以后也许会优化的。 但现在，作者狠不得把114个小时掰成514个小时来用    

**（上文为“入门部分”。）
（如果这是您第一次使用此 mod，那您可以在此处停止阅读，然后去玩“边缘世界”了。**

------------------------------------------

## 已知问题:
 暂时未知(截至 20240809)    

## 更新日志:
 - 20240915 - v1.0.1.1-beta    
   - 第一次github发布    


## 参考文献:
（占位符）

**(上文为"进阶部分（极客部分）")**

------------------------------------------

## 作者的未来计划:
- (积压) 割人机 (近战):    
  一种加装腿部额外装甲和内置的更强的近战技能与更弱园艺技能的的"园艺机" （CE 中的园艺机就是小蜈蚣，20RHA的穿甲呢，可惜腿部太脆弱）  

## 致谢: （排名不分先后）    
 - **!! 感谢所有社区贡献者与社区享受者，和你 - 鼓励支持与bug反馈 !!**    
 - **!! 感谢所有在贴图方面提出建议的好同志们 !!**
 - **[!! PlasmaBurst2 - 灵感来源之一 !! (点此访问他们的页面)](https://www.plazmaburst2.com/)**    

## 关于作者们:
 - SawedoffShotgun

## 版权/许可:
 Rimworld is owned by Tynan Sylvester.    
 <p xmlns:cc="http://creativecommons.org/ns#" xmlns:dct="http://purl.org/dc/terms/"><span property="dct:title">CEHDDPS</span> by <a rel="cc:attributionURL dct:creator" property="cc:attributionName" href="http://mailto:youngturksfree@outlook.com">SawedoffShotgun</a> is licensed under <a href="https://creativecommons.org/licenses/by-nc-nd/4.0/?ref=chooser-v1" target="_blank" rel="license noopener noreferrer" style="display:inline-block;">CC BY-NC-ND 4.0 (Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License)<img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/cc.svg?ref=chooser-v1" alt=""><img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/by.svg?ref=chooser-v1" alt=""><img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/nc.svg?ref=chooser-v1" alt=""><img style="height:22px!important;margin-left:3px;vertical-align:text-bottom;" src="https://mirrors.creativecommons.org/presskit/icons/nd.svg?ref=chooser-v1" alt=""></a></p>    

## 联系方式: (如遇问题，请联系作者电子邮箱)
 联系方式:  
  [电子邮件Email(主要方式): youngturksfree@outlook.com](mailto:youngturksfree@outlook.com)    
  [Github: https://github.com/AlohaOe114](https://github.com/AlohaOe114)    
 Ludeon(官方论坛): SawedoffShotgun    

## 另见:(其他作品)    
 [SawedoffShotgun's Gun_Misc]()    
 [SawedoffShotgun's MeleeWeapons_Misc]()    

## 如果您喜欢我的作品
(也许能鞭策作者能进行一个更肝更快的更新)        
~~[如果您喜欢我的作品，那么能否帮我点杯ko-fi]~~      
~~[爱发电]~~      
~~[Patreon]~~

暂不接委托    

![Preview](/About/Preview.png)